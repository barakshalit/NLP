...
public class LdaGibbsSampler {
    ...
    private void gibbs(int K, double alpha, double beta) {
        ...
        for (int i = 0; i < ITERATIONS; i++) {
            ...
            if ((i % 500) == 0) {
               try {
               System.out.println("writing z " + i + ".");
               PrintWriter zout = new PrintWriter(new FileOutputStream(new File("moviekw.z." + i)));
               writeZ(zout);
               zout.close();
    ...
    public void writeDist(double[][] dist,PrintWriter out) {
	...
	out = new PrintWriter(new FileOutputStream(new File(args[0]+".nolem.doctopic.dist")));
	lda.writeDist(theta, out);

	out = new PrintWriter(new FileOutputStream(new File(args[0]+".nolem.topicword.dist")));
	lda.writeDist(phi, out);
